...
默认情况下，Web3 管理多层中间件。它们位于公共 Web3 方法和 [提供者](providers.html) 之间，后者处理与以太坊客户端的本地通信。每一层都可以修改请求和/或响应。有些中间件是默认启用的，有些是可选的。

每个中间件层在请求到达提供者之前被调用，然后在提供者返回之后以相反的顺序处理结果。然而，中间件有可能在请求没有到达提供者(甚至没有到达更深层的中间件)的情况下就从调用中提前返回。
...
该中间件仅在通过 mainnet(部署了 ENS 合同的地方)调用时转换 ENS 名称，对于所有其他情况，将导致一个`InvalidAddress`错误
...
如果适用，并且已经设定了天然气价格策略，这会将天然气价格添加到交易中。参见 [气价 API](gas_price.html#gas-price) 了解气价是如何得出的信息。
