...
> 原文:[https://web3py.readthedocs.io/en/stable/contracts.html](https://web3py.readthedocs.io/en/stable/contracts.html)
 ...
 智能合同是部署到以太网的程序。参见[ethereum.org 文件](https://ethereum.org/en/developers/docs/smart-contracts)以获得适当的介绍。
...
这些工厂不打算直接初始化。相反，使用 [`w3.eth.contract()`](web3.eth.html#web3.eth.Eth.contract "web3.eth.Eth.contract") 方法创建契约对象。默认情况下，合同工厂为 [`Contract`](#web3.contract.Contract "web3.contract.Contract") 。参见 [`ConciseContract`](#web3.contract.ConciseContract "web3.contract.ConciseContract") 中指定替代工厂的示例。
...
这个 [`Contract`](#web3.contract.Contract "web3.contract.Contract") 的变体是为更简洁的读访问而设计的，不会使写访问更冗长。这是以失去对像`deploy()`这样的功能和像`address`这样的属性的访问为代价的。对于那些用例，建议使用经典的`Contract`。需要明确的是，<cite>construct Contract</cite>只公开契约函数，所有其他<cite>契约</cite>类方法和属性在<cite>construct Contract</cite>API 中不可用。这包括但不限于`contract.address`、`contract.abi`和`contract.deploy()`。

通过将一个 [`Contract`](#web3.contract.Contract "web3.contract.Contract") 实例传递给 [`ConciseContract`](#web3.contract.ConciseContract "web3.contract.ConciseContract") 来创建这种类型的合同:

```
>>> concise = ConciseContract(myContract) 
```

这个变体以调用的方式调用所有方法，所以如果经典契约有类似于`contract.functions.owner().call()`的方法，您可以用`concise.owner()`来调用它。
...
这个变体镜像了 [`ConciseContract`](#web3.contract.ConciseContract "web3.contract.ConciseContract") ，但是它调用所有方法作为一个事务而不是一个调用，所以如果经典契约有一个类似于`contract.functions.owner.transact()`的方法，你可以用`implicit.owner()`来代替。
...
这提供了对作为属性的合同功能的访问。比如:`myContract.functions.MyMethod()`。公开的契约函数是类型为 [`ContractFunction`](#web3.contract.ContractFunction "web3.contract.ContractFunction") 的类。

```
Contract.events
```

这提供了对作为属性的合同事件的访问。比如:`myContract.events.MyEvent()`。公开的契约事件是类型为`ContractEvent`的类。
...
*   `address`可选。默认为合同地址。过滤器匹配从`address`发出的事件日志。
...
如果契约接受构造函数参数，它们应该通过`args`参数以列表的形式提供。
...
使用以太坊 ABI 对与给定的`fn_name`和参数`args`相匹配的契约函数的参数进行编码。`data`参数默认为功能选择器。
...
返回合同中所有函数的列表，其中每个函数都是 [`ContractFunction`](#web3.contract.ContractFunction "web3.contract.ContractFunction") 的实例。
...
`Contract`方法`all_functions`、`get_function_by_signature`、`find_functions_by_name`、`get_function_by_name`、`get_function_by_selector`、`find_functions_by_args`、`get_function_by_args`只有在 abi 提供给合同时才能使用。

注意

Web3.py 拒绝初始化具有相同选择器或签名的多个函数的契约。例如，`blockHashAddendsInexpansible(uint256)`和`blockHashAskewLimitary(uint256)`具有与`0x00000000`相同的选择器值。包含这两种功能的合同将被拒绝。  ## 调用不明确的契约函数示例

下面是一个合同的例子，它有多个同名的功能，并且参数不明确。
...
[`ContractFunction`](#web3.contract.ContractFunction "web3.contract.ContractFunction") 提供了与合同函数进行交互的方法。提供给约定函数子类的位置和关键字参数将用于通过签名查找约定函数，并在适用时转发给约定函数。
...
函数调用的第一部分`myMethod(*args, **kwargs)`根据名称和提供的参数选择合适的契约函数。参数可以以位置参数、关键字参数或两者混合的形式提供。
...
调用一个契约函数，使用`eth_call` API 在本地执行事务。这不会创建新的公共事务。
...
> 合同工厂还提供了与 fallback 函数交互的 API，它支持普通函数等四种方法:
...
`ContractEvent`提供与合同事件交互的方法。提供给约定事件子类的位置和关键字参数将用于通过签名查找约定事件。
...
`ContractCaller`类提供了一个 API 来调用契约中的函数。这个类不是直接使用，而是通过`Contract.caller`使用。
...
与 [`ContractFunction`](#web3.contract.ContractFunction "web3.contract.ContractFunction") ， [`ContractCaller`](#web3.contract.ContractCaller "web3.contract.ContractCaller") 一样，提供了与契约函数进行交互的方法。提供给协定调用方子类的位置和关键字参数将用于通过签名查找协定函数，并在适用时转发给协定函数。

## 合同常见问题

### 如何将结构作为函数参数传入？

Web3.py 接受结构参数作为字典。这种格式也支持嵌套结构。让我们看一个简单的例子。给定以下可靠性合同:
