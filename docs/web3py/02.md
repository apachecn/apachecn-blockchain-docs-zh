...
该库依赖于与以太坊节点的连接。我们将这些连接称为*提供者*，有几种方法可以配置它们。完整的细节可以在 [供应商](providers.html#providers) 文档中找到。该快速入门指南将重点介绍几个最常见的使用案例。

### [测试提供者](#id4)

如果你只是在学习诀窍或者做一些快速原型制作，你可以使用一个测试提供者， [eth-tester](https://github.com/ethereum/eth-tester) 。该提供程序包括一些预先填充了测试以太网的帐户，并将每个交易自动生成一个块。Web3.py 通过`EthereumTesterProvider`使这个测试提供程序可用:
...
如果您坚持使用默认的端口或 IPC 文件位置，您可以利用一个 [便利方法](providers.html#automatic-provider) 来自动检测提供者并节省一些击键次数:
...
与以太坊区块链互动的最快方式是使用远程节点提供者，如 [Infura](https://infura.io/) 、 [Alchemy](https://www.alchemy.com/) 或 [QuickNode](https://www.quicknode.com/) 。您可以通过指定端点来连接到远程节点，就像前面的本地节点示例一样:
...
Web3.py 可以帮助您读取块数据、签署和发送事务、部署合同并与之交互，以及许多其他功能。

你想做的许多典型的事情都在 [`w3.eth`](web3.eth.html#web3.eth.Eth "web3.eth.Eth") API 中，所以这是一个很好的起点。

如果您想直接进入契约，请查看关于 [契约](contracts.html#contracts) 的部分，包括一个 [契约部署示例](contracts.html#contract-example) ，以及如何使用 [`w3.eth.contract()`](web3.eth.html#web3.eth.Eth.contract "web3.eth.Eth.contract") 创建一个契约实例。
