...
*   [使用合同](#working-with-contracts)
    *   [与现有合同互动](#interacting-with-existing-contracts)
    *   [部署新合同](#deploying-new-contracts)
*   [通过 ethPM 处理合同](#working-with-contracts-via-ethpm)
*   [使用 ERC20 令牌合同](#working-with-an-erc20-token-contract)
    *   [创建合同工厂](#creating-the-contract-factory)
...
*   [Python 中的契约单元测试](#contract-unit-tests-in-python)
...
*   [T2`send_raw_transaction()`](web3.eth.html#web3.eth.Eth.send_raw_transaction "web3.eth.Eth.send_raw_transaction")
    ...
    *   您希望在其他地方签署交易，例如硬件钱包。
    *   您希望通过另一个提供者(例如 Infura)广播一个事务。
    *   您还有其他一些需要更多灵活性的高级用例。

*   [合同功能](contracts.html#contract-functions)

    Use these methods if:

    *   你想与合同互动。Web3.py 解析契约 ABI，并通过`functions`属性使这些函数可用。
...
为了使用现有的合同，您需要它的部署地址和 ABI。两者都可以使用块浏览器找到，如 Etherscan。一旦实例化了一个契约实例，就可以读取数据和执行事务。
...
[ethPM](http://www.ethpm.com/) 包包含已配置好的合同，随时可以使用。Web3 的`ethpm`模块(`web3.pm`)扩展了 Web3 的本机`Contract`模块，对如何实例化`Contract`工厂和实例做了一些修改。
...
`web3.pm`使用`Package`类来表示一个 ethPM 包。这个对象在一个包中包含了所有的契约资产，并通过一个 API 公开它们。因此，在我们能够与我们的包交互之前，我们需要将它生成为一个`Package`实例。
...
现在我们有了目标 ethPM 包的一个`Package`表示，我们可以从这个`Package`生成契约工厂和实例。但是，需要注意的是，有些包可能缺少生成实例或工厂所需的必要契约资产。您可以使用 [ethPM CLI](https://github.com/ethpm/ethpm-cli) 来确定 ethPM 包中可用的合同类型和部署。
...
以太坊区块链上的大多数可替换代币都符合 [ERC20](https://github.com/ethereum/EIPs/blob/7f4f0377730f5fc266824084188cc17cf246932e/EIPS/eip-20.md) 标准。指南的这一部分介绍了如何与符合此标准的现有令牌合同进行交互。

在本指南中，我们将与已经部署到本地测试链的现有令牌合约进行交互。本指南假设:

1.  已知地址的现有令牌协定。
2.  对给定合同的适当`ABI`的访问。
3.  一个`web3.main.Web3`实例连接到一个提供者，该提供者拥有一个可以发送交易的未锁定帐户。

### [创建合同工厂](#id14)

首先，我们需要用令牌契约的地址和`ERC20` ABI 创建一个契约实例。
...
每个代币将有一个代表流通代币总数的总供应量。在本例中，我们将令牌合约初始化为拥有 100 万个令牌。由于该令牌合同设置为具有 18 位小数，因此该合同返回的原始总供应量将具有 18 位额外的小数。
...
接下来，我们可以使用契约的`balanceOf`函数查询一些账户余额。我们正在使用的令牌合约从一个帐户开始，我们称之为`alice`持有所有令牌。
...
接下来，我们可以使用契约的`transfer`函数将一些令牌从`alice`转移到`bob`。
...
*   只支持`HTTPS`提供者，因为 JSON-RPC 重试逻辑依赖于底层协议的实现细节，
