...
*   通过`validate_payable()`合约方法( [#2609](https://github.com/ethereum/web3.py/issues/2609) )验证时允许`value`字段为十六进制
...
*   在 ENS 中，解析 ENS 地址的合约函数被错误地调用了两次。其中一个电话被删除了。( [#2318](https://github.com/ethereum/web3.py/issues/2318) )
...
*   将<cite>输出值</cite>显式添加到合约示例( [#2293](https://github.com/ethereum/web3.py/issues/2293) )
*   更新<cite>异步供应商</cite>样本代码的导入( [#2302](https://github.com/ethereum/web3.py/issues/2302)
...
*   修复异步供应商文档中的错别字( [#2131](https://github.com/ethereum/web3.py/issues/2131) )
...
*   澄清一些合约`ValueError`错误信息。( [#2146](https://github.com/ethereum/web3.py/issues/2146) )
...
*   合约函数调用解码异常的字符串插值问题的热修复，以便于从 eth_call 响应中提取有意义的消息( [#2096](https://github.com/ethereum/web3.py/issues/2096) )
...
*   不再维护将`py-solc-x`更新为`solc`的“合约部署示例”文档。( [#2020](https://github.com/ethereum/web3.py/issues/2020) )
...
*   支持合约函数调用中的 state_override。( [#2005](https://github.com/ethereum/web3.py/issues/2005) )
...
*   添加不稳定异步供应商的文档( [#2017](https://github.com/ethereum/web3.py/issues/2017) )
...
*   将现有合约交互添加到文档示例中。( [#1933](https://github.com/ethereum/web3.py/issues/1933) )
...
*   增加了一个新的`ContractLogicError`用于当合约回复交易时。在 v6 中，`ContractLogicError`将取代`SolidityError`。( [#1814](https://github.com/ethereum/web3.py/issues/1814) )
...
*   现代化部署合约示例。( [#1679](https://github.com/ethereum/web3.py/issues/1679) )
...
*   在基于时间的燃气价格策略中增加了新的加权关键字参数。

    如果`True`，它将给予最近的阻塞时间更多的权重。( [#1614](https://github.com/ethereum/web3.py/issues/1614) )

*   增加了对 Solidity 新的(ish)接收功能的支持。

    添加镜像现有回退 API 的新合约 API: `contract.receive` ( [#1623](https://github.com/ethereum/web3.py/issues/1623) )
...
*   更新合约事件文档以显示正确的示例( [#1515](https://github.com/ethereum/web3.py/issues/1515) )
...
*   混杂的
    *   [ethPM]更新默认注册管理机构可靠性合约- [#1400](https://github.com/ethereum/web3.py/pull/1400)
...
*   特征
    *   在供应商 IPC 路径中添加对 tilda 的支持- [#1049](https://github.com/ethereum/web3.py/pull/1049)
    *   支持 EIP 712 签约- [#1319](https://github.com/ethereum/web3.py/pull/1319)
*   文件（documents 的简写）
    *   更新合约示例以使用`compile_standard` - [#1263](https://github.com/ethereum/web3.py/pull/1263)
...
*   特征
    ...
    *   增加了对 Goerli 供应商的支持- [#1286](https://github.com/ethereum/web3.py/pull/1286)
    *   将 addReservedPeer 添加到奇偶校验模块- [#1311](https://github.com/ethereum/web3.py/pull/1311)
*   错误修复
    *   将燃气价格值转换为燃气策略中的整数- [#1297](https://github.com/ethereum/web3.py/pull/1297)
...
*   特征
    ...
    *   合约调用者 API 已实现- [#1227](https://github.com/ethereum/web3.py/pull/1227)
...
*   混杂的
    ...
    *   将测试者供应商结果从中间件迁移到默认值- [#1188](https://github.com/ethereum/web3.py/pull/1188)
...
*   特征
    ...
    *   拒绝将以太网发送到非应付合约功能的交易- [#1115](https://github.com/ethereum/web3.py/pull/1115)
...
*   特征
    ...
    *   合约调用优化- [#944](https://github.com/ethereum/web3.py/pull/944)
*   错误修复
...
*   混杂的
    ...
    *   文档:添加与 ERC20 合约交互的示例- [#995](https://github.com/ethereum/web3.py/pull/995)
...
*   特征
    ...
    *   解决模糊函数调用的新 API，例如:
        ...
        *   请看如何在: [调用不明确的合约函数示例](contracts.html#ambiguous-contract-functions)
*   错误修复
    *   气体估计不崩溃，当 0 块可用。(即~在创世纪板块上)
    *   关闭所有 HTTPProvider 会话，以消除退出时的警告
    *   停止向筛选器添加两次合约地址。这让一些节点不高兴
*   混杂的
...
*   添加了一个 API 来查找合约函数:`fn = contract.functions['function_name_here']`
...
*   更好的错误消息，当试图使用一个合约功能有 0 或 1 以上的匹配
...
*   现在，您可以使用`block_identifier`关键字参数在特定块调用合约方法，参见: [`call()`](contracts.html#web3.contract.ContractFunction.call "web3.contract.ContractFunction.call")
*   在准备稳定发布时，默认禁用`w3.eth.account`，直到第三方审计完成&解决。
*   用于合约部署的新 API，支持燃气估算、本地签署等。 [`constructor()`见](contracts.html#web3.contract.Contract.constructor "web3.contract.Contract.constructor")。
*   用[contract . events . $ my _ event . create filter()](contracts.html#contract-createfilter)查找合约事件
*   支持合约方法的自动完成。
*   将大多数依赖项升级到稳定
    *   eth-abi
    *   eth-utils
...
*   设置环境变量 <cite>WEB3_PROVIDER_URI</cite> 以建议自动检测的供应商
*   设置供应商的新 API，如:`w3.providers = [IPCProvider()]`
...
*   [`contract()`](web3.eth.html#web3.eth.Eth.contract "web3.eth.Eth.contract") 接受 ENS 名称作为合约地址
...
*   合约`eventFilter()`已经取代了`Contract.on()`和`Contract.pastEvents()`
*   ABI 类型的合约参数现在接受十六进制字符串。
*   `string` ABI 类型的合约参数现在接受 python `str`。
*   `string` ABI 类型的合约返回值现在返回 python `str`。
...
*   增加对`eth`的`abi`参数的验证
*   `bytes`、`bytesXX`和`string`的合约返回值不再转换为文本类型，而是以其原始的字节-字符串格式返回。
...
*   修正了合约实例尊重`web3.eth.defaultAccount`的错误
*   更好的错误报告时，ABI 解码失败的合约方法响应。
...
*   修正了合约交易发送不考虑`web3.eth.defaultAccount`配置的错误。
...
*   从合约`on`和`pastEvents`方法返回的过滤器现在使用与`web3.js`相同的数据格式调用它们的回调。
