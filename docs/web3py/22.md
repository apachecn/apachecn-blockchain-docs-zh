...
*   `data` : `bytes or text` -合约的编译代码或调用的方法签名和编码参数的散列。详见[以太坊合约 ABI](https://github.com/ethereum/wiki/wiki/Ethereum-Contract-ABI) 。
...
*   如果设置了燃气价格策略，将使用燃气价格策略中的`gasPrice`值(参见 [燃气价格 API](gas_price.html#gas-price) )。
...
大多数情况下最好通过 [`web3.contract.Contract`](contracts.html#web3.contract.Contract "web3.contract.Contract") 接口进行合约函数调用。
...
`transaction_params`参数是可选的，但是一些燃气价格策略可能要求 it 能够生成燃气价格。
...
有关如何在 web3 中定制燃气价格的信息，请参见 [燃气价格 API](gas_price.html#gas-price) 。
...
*   `address` : `string`或`strings`列表，每个 20 字节-(可选)合约地址或日志应源自的地址列表。
...
如果提供了`address`，那么这个方法将返回由`abi`定义的合约的一个实例。该地址可能是一个校验和字符串，或者是一个类似`'mycontract.eth'`的 ENS 名称。
...
如果您使用 ENS 名称来初始化合约，则每次使用时将按名称查找合约。如果名称可能被恶意更改，首先 [查找 ENS 名称](ens_overview.html#ens-get-address) 的地址，然后使用校验和地址创建合约。

如果`address`为*未提供*，则返回新创建的合约类。然后，通过提供地址来初始化该类。
...
`contract_name`将被用作合约类的名称。如果是`None`，那么将使用`ContractFactoryClass`的名称。

`ContractFactoryClass`将被用作基本合约类。
...
关于如何使用合约的更多信息，请参见 [合约](contracts.html) 。

```
Eth.set_contract_factory(*contractFactoryClass*)
```

将默认合约工厂从`Contract`修改为`contractFactoryClass`。未来对`Eth.contract()`的呼叫将默认为`contractFactoryClass`。

替代合约工厂的一个例子是`ConciseContract`。
