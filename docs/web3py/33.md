...
*   客户端目录中的文件包含所有提供者测试(http、ipc 和 ws)共享的代码。这主要用于覆盖跨越所有提供者的测试。
...
*   `test_*client*_*provider*.py`(例如`test_goethereum_http.py`)文件是客户和供应商特定的测试配置存在的地方。这主要用于覆盖特定于各个客户端的提供者类型的测试。
...
“设备”是一个预先同步的网络。它是配置和运行客户机、部署测试契约以及保存测试 Web3.py 功能的结果。
