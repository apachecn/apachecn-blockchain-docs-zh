...
<cite>Py-EthPM</cite> 使用自定义子类`Web3.contract.Contract`来管理可能需要字节码链接的合同工厂和实例。为了创建一个可部署的契约工厂，契约类型的`abi`和`deploymentBytecode`必须在包的清单中可用。
...
部署字节码的链接引用数据列表，如果出现在用于生成`LinkableContract`工厂的清单数据中。部署字节码链接引用数据必须出现在清单中，以便为需要字节码链接的契约生成工厂。
...
该方法返回一个新创建的合同工厂，其中应用了在`attr_dict`中定义的链接引用。对于所有未链接的链接引用，该方法期望`attr_dict`的类型为`Dict[`contract_name`: `address`]`。
...
从注册表中查找包的 URI 应该遵循以下格式。(随着注册管理机构合同标准进入 EIP 流程，可能会有所变化)
...
*   `address`:(必需)必须是有效的 ENS 域或指向注册管理机构合同的有效校验和地址。
...
要为 solidity 契约构建更复杂的清单，需要从 solidity 编译器提供标准的 json 输出。或者为了更方便的体验，使用 [EthPM CLI](https://github.com/ethpm/ethpm-cli) 。

下面是一个如何编译合同并生成标准 json 输出的示例。更多信息可以在 [Solidity 编译器](https://solidity.readthedocs.io/en/v0.4.24/using-the-compiler.html)文档中找到。
...
以下示例中使用的`compiler_output`是 solc 输出的`contracts`键的整个值，它包含所有已编译契约的编译数据。
...
要在清单中直接内联源代码，使用`inline_source()`或`source_inliner()`(内联来自同一个 compiler_output 的多个源代码)，这需要契约名称和编译器输出作为参数。
...
要将源作为内容寻址 URI 包含在内，`Py-EthPM`可以通过 Infura IPFS API 锁定您的源。除了合同名称和编译器输出，该函数还要求您提供所需的 IPFS 后端来固定合同源。
...
清单生成器的`contract_type()`函数的默认行为是用在`compiler_output`中找到的所有合同类型数据填充清单。
...
如果您想为您的合同类型起别名，请以 kwarg 的形式提供所需的别名。这将自动在`contractType`字段中包含原始合同类型。除非以 kwargs 的形式提供特定的合同类型字段，`contractType`仍将默认包含编译器输出中找到的所有可用的合同类型数据。
...
根据 [EthPM 规范](http://ethpm.github.io/ethpm-spec/package-spec.html#the-contract-instance-object)，所有的`runtime_bytecode`、`deployment_bytecode`和`compiler`都必须是有效格式的字典。如果您的契约有链接依赖，请确保将它们包含在字节码对象中。
...
此构建器功能简化了跨多个链添加相同合同类型部署的过程。它需要一个`contract_instance`和`contract_type`参数(在许多情况下，这两个参数是相同的，尽管`contract_type` *必须*总是匹配其在清单的“合同类型”中的对应方)，并且所有参数都需要关键字。
